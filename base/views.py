...
def loginPage(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, 'Böyle bir Kullanıcı Adı yok')
...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Kayıt esnasında bir hata oluştu.')
...
def updateRoom(request, pk):
     ...
     if request.user != room.host:
        return HttpResponse('Bunu yapmak için yetkili değilsiniz.')
